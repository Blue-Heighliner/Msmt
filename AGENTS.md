...
- Never add a `<Version>` to `Core/Core.csproj` — the only way to release is `Tasks/Publish.task`, which creates the tag/GitHub Release locally (a repo ruleset blocks `GITHUB_TOKEN` from creating tags, so CI can't do this part) and then dispatches the `csharp.yml` workflow (`workflow_dispatch`, restricted to repo Admins/org Owners) with a plain `Major.Minor.Patch[-prerelease]` version (no leading `v`, e.g. `1.2.3`) to pack, publish, and attach assets to that release. Pushing a tag directly does not publish anything.
